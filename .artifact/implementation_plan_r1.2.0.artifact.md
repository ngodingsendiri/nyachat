...
| 4.6 | Commit + tag | `fix: r1.2.0 ...` → push tag `r1.2.0` → GitHub Release | 🔶 **SIAP DI-EKSEKUSI 2026-08-10** — auth git TERVERIFIKASI via Windows Credential Manager (`git ls-remote origin` berhasil tanpa prompt); workflow CI otomatis membuat GitHub Release saat tag `r*` di-push (permissions contents: write). `gh` CLI tetap tidak ada & `GITHUB_TOKEN` kosong → tidak bisa upload aset ke release manual via API (hanya bisa via CI). HEAD siap = `48dd3ee` (fix/r1.2.0). |

...
| 4.5 | CI penuh | `testDebugUnitTest` + `lintDebug` + `verifyRoborazziDebug` + lint firestore rules | ✅ **SELESAI 2026-08-10** — unit test PASS, **verifyRoborazzi 0 failure** (setelah re-record 4.4), lintDebug 0 error, lint rules (eslint firestore.rules) PASS. **Re-verified 2026-08-10 (pasca polish UI)**: BUILD SUCCESSFUL untuk ketiganya + eslint PASS — state 48dd3ee siap rilis |
| 4.6 | Commit + tag | `fix: r1.2.0 ...` → push tag `r1.2.0` → GitHub Release | ⏸️ **DITUNDA 2026-08-10 (keputusan user)** — jangan rilis dulu: user masih menemukan beberapa bug yang perlu diperbaiki. Persiapan sudah lengkap: auth git via Windows Credential Manager terverifikasi (ls-remote OK), CI auto-release pada tag `r*`, HEAD siap = commit terbaru fix/r1.2.0. Lanjutkan saat bug selesai diperbaiki. |
| 4.6b | **Koreksi release r1.1.3** (deferral FASE 0.5) | Upload APK r1.1.3/26 yang benar ke release r1.1.3 — bersamaan dengan rilis r1.2.0 | ⏸️ **DITUNDA** mengikuti 4.6 — butuh kredensial GitHub API (gh CLI/GITHUB_TOKEN) untuk upload aset ke release existing |
| 4.7 | Play Store checklist | Jalankan `docs/PLAY_STORE_CHECKLIST.md`; upload AAB release | ⏸️ **DITUNDA** mengikuti 4.6 — butuh keystore upload (secret CI) & kredensial Play Console; langkah manual sudah ada di `docs/PLAY_STORE_CHECKLIST.md` |
...
- **Polish UI lanjutan pasca-finalisasi (2026-08-10)**: warna income dark dilembutkan `#69EFC4`→`#8FC6AD` (commit `54d4fb1`, pixel-verified 636px sage/0px lama); **FAB jump-to-bottom tidak lagi menutupi bubble chat terakhir saat scroll** (commit `48dd3ee` — baris chips dipindah ke dalam Box daftar sehingga FAB melayang di atas chips, overlap terukur 110×16px → 0, endPadding chips 64dp, FAB sembunyi saat mengetik; verified live emulator-5554). Keduanya sudah masuk CHANGELOG & CI hijau (4.5 re-verified).
